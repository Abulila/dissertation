...
As a consequence, persists from epochs that race have no defined order (they are concurrent), but persists from prior and subsequent epochs are ordered.
Additionally, persists to the same address occur in the order observed by cache coherence, allowing ordered persists even when persist epochs race.
